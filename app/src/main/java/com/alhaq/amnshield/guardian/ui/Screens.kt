...
import com.alhaq.amnshield.guardian.ui.components.ToggleItem
        ...
        blocks.forEach { b ->
            Card(Modifier.padding(bottom = 8.dp)) {
                Column(Modifier.padding(12.dp)) {
                    ...
                    Text("Apps: ${"$"}{b.appIds.size} â€¢ Websites: ${"$"}{b.websites.size} â€¢ Keywords: ${"$"}{b.keywords.size}")
                }
            }
        }
    }
...
@Composable
fun AppBlocksScreen(vm: BlockViewModel) {
    val ctx = LocalContext.current
    val appRules = vm.appRules
    ...
    LazyColumn(
        ...
        val filtered = appRules.filter { rule ->
            (!rule.isSystem || showSystemApps) && 
            (search.isEmpty() || rule.appName.contains(search, ignoreCase = true) || rule.packageName.contains(search, ignoreCase = true))
        }
        
        if (filtered.isEmpty()) {
            item {
                Text(
                    if (appRules.isEmpty()) "Loading apps..." else "No apps found",
                    modifier = Modifier.fillMaxWidth(),
        ...
        items(filtered) { rule ->
            AppRuleCard(rule = rule, onUpdate = { updated ->
                vm.updateAppRule(updated)
            })
        }
    }
}
...
@Composable
private fun AppRuleCard(rule: com.alhaq.amnshield.guardian.model.AppRule, onUpdate: (com.alhaq.amnshield.guardian.model.AppRule) -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { /* Expand details */ },
        colors = if (rule.isBlocked())
        ...
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Row(
                ...
                Column(Modifier.weight(1f)) {
                    ...
                    Text(rule.packageName, style = MaterialTheme.typography.bodySmall, fontSize = 10.sp)
                }
                if (rule.isSystem) {
                    AssistChip(onClick = {}, label = { Text("System", fontSize = 9.sp) })
                }
            }
            ...
            Row(
                ...
                Column(Modifier.weight(1f)) {
                    Text("Block WiFi", style = MaterialTheme.typography.labelSmall)
                }
                Checkbox(
                    checked = rule.wifiBlocked,
                    onCheckedChange = { onUpdate(rule.copy(wifiBlocked = it)) }
                )
            }
            ...
            Row(
                ...
                Column(Modifier.weight(1f)) {
                    Text("Block Mobile", style = MaterialTheme.typography.labelSmall)
                }
                Checkbox(
                    checked = rule.mobileBlocked,
                    onCheckedChange = { onUpdate(rule.copy(mobileBlocked = it)) }
                )
            }
            ...
            if (rule.isBlocked()) {
                ...
                Row(
                    ...
                    Column(Modifier.weight(1f)) {
                        Text("Allow WiFi when screen on", style = MaterialTheme.typography.labelSmall, fontSize = 10.sp)
                    }
                    Checkbox(
                        checked = rule.allowWifiWhenScreenOn,
                        onCheckedChange = { onUpdate(rule.copy(allowWifiWhenScreenOn = it)) }
                    )
                }
                ...
                Row(
                    ...
                    Column(Modifier.weight(1f)) {
                        Text("Allow Mobile when screen on", style = MaterialTheme.typography.labelSmall, fontSize = 10.sp)
                    }
                    Checkbox(
                        checked = rule.allowMobileWhenScreenOn,
                        onCheckedChange = { onUpdate(rule.copy(allowMobileWhenScreenOn = it)) }
                    )
                }
                ...
                Row(
                    ...
                    Column(Modifier.weight(1f)) {
                        Text("Block when roaming", style = MaterialTheme.typography.labelSmall)
                    }
                    Checkbox(
                        checked = rule.blockWhenRoaming,
                        onCheckedChange = { onUpdate(rule.copy(blockWhenRoaming = it)) }
                    )
                }
                ...
                Row(
                    ...
                    Column(Modifier.weight(1f)) {
                        Text("Lockdown (block all)", style = MaterialTheme.typography.labelSmall)
                    }
                    Checkbox(
                        checked = rule.lockdown,
                        onCheckedChange = { onUpdate(rule.copy(lockdown = it)) }
                    )
                }
            }
        }
...
private fun ServiceControls(vm: BlockViewModel) {
    ...
    val permissionLauncher = rememberLauncherForActivityResult(
        ...
        vm.checkApiPermission()
    }

    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Text("Blocker Connection", style = MaterialTheme.typography.titleMedium)

        Card(
            modifier = Modifier.fillMaxWidth(),
            colors = CardDefaults.cardColors(
                containerColor = if (apiConnected) 
            ...
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    if (!apiConnected) {
                        Button(onClick = { vm.reconnectApi() }) { Text("Connect") }
                    } else {
                        OutlinedButton(onClick = { vm.connectionManager.disconnect() }) { Text("Disconnect") }
                    }
                }
            }
        }
        ...
        Card(
            ...
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Row(
                    ...
                    Column(modifier = Modifier.weight(1f)) {
                        Text("Blocker API Permission", style = MaterialTheme.typography.titleSmall)
                        Text(
                            if (isGranted) "Authorized to manage blocker settings" else "Authorize this app in AmnShield",
                            style = MaterialTheme.typography.bodySmall,
                            color = if (isGranted) 
                                MaterialTheme.colorScheme.onPrimaryContainer 
                            else 
                ...
                if (!isGranted) {
                    Button(onClick = {
                        val intent = vm.requestApiPermission()
                        if (intent != null) {
                            try {
                                permissionLauncher.launch(intent)
                            } catch (e: Exception) {
                                android.util.Log.e("Screens", "Failed to launch permission activity", e)
                            }
                        } else {
                            vm.reconnectApi()
                        }
