...
import org.alhaq.deenshield.guardian.BlockerApplication
import org.alhaq.deenshield.guardian.MainActivity
import org.alhaq.deenshield.guardian.R
import org.alhaq.deenshield.guardian.network.ContentFilter
import org.alhaq.deenshield.guardian.network.PacketParser
import org.alhaq.deenshield.guardian.network.DnsProxy
...
class BlockingVpnService : VpnService() {
    ...
    override fun onCreate() {
        ...
        contentFilter.updateConfiguration(
            domains = org.alhaq.deenshield.guardian.util.BlockLists.harmfulWebsites,
            keywords = org.alhaq.deenshield.guardian.util.BlockLists.harmfulKeywords,
            socialMedia = false,
            adultContent = true,
            gambling = true
        )
        ...
        try {
            while (isActive && currentCoroutineContext().isActive) {
                try {
                    ...
                    if (isActive) {
                        android.util.Log.w("BlockingVpnService", "Error processing packet: ${e.message}", e)
                        // Continue processing on recoverable errors
                        try {
                            delay(50) // Brief pause to avoid tight loop on persistent errors
                        } catch (de: Exception) {
    ...
    private suspend fun analyzePacket(packet: org.alhaq.deenshield.guardian.network.IpPacket): Boolean {
    ...
    private fun updateConfiguration(intent: Intent) {
        ...
        val mergedDomains = if (domains != null) {
            (domains.toSet() + org.alhaq.deenshield.guardian.util.BlockLists.harmfulWebsites)
        } else {
            org.alhaq.deenshield.guardian.util.BlockLists.harmfulWebsites
        }
        ...
        contentFilter.updateConfiguration(
            ...
            keywords = if (blockKeywords) org.alhaq.deenshield.guardian.util.BlockLists.harmfulKeywords else emptySet(),
    ...
    private fun sendBlockNotification(event: org.alhaq.deenshield.guardian.network.BlockEvent) {
    ...
    private fun buildIpv4UdpResponse(request: org.alhaq.deenshield.guardian.network.IpPacket, udpPayload: ByteArray): ByteArray? {
    ...
    companion object {
        ...
        const val ACTION_START = "org.alhaq.deenshield.guardian.vpn.START"
        const val ACTION_STOP = "org.alhaq.deenshield.guardian.vpn.STOP"
        const val ACTION_UPDATE_CONFIG = "org.alhaq.deenshield.guardian.vpn.UPDATE_CONFIG"
